...
const DisplayDessertsDropdown = ({ desserts, fetchRequest }) => {
  ...
  function fetchFunction(e, params) {
    e.preventDefault();
    fetch(
      `https://dessert1-app.herokuapp.com/desserts/${selectedDessert[0].id}`,
      {
        method: requestType,
  ...
  return (
    <div>
      <FormControl className="menu">
        <InputLabel id="demo-simple-select-label" className="dessert-title">
          Select Dessert
        </InputLabel>
        <Select
        ...
        >
          ...
          {desserts.map((dessert) => (
            <MenuItem key={dessert.id} value={dessert}>
              {dessert.name}
            </MenuItem>
          ))}
        </Select>
